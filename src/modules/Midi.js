...
class Midi {
	...
	decode(bytes) {
		...
		let tmpTimeSignatures = [];
		for(let i in gsMusicObject.timeSignatures){
			let tmpTimeSig = gsMusicObject.timeSignatures[i];
			// let isUnique = 
			// for(let j in tmpTimeSignatures){

			// }
			tmpTimeSig = tmpTimeSig.split('/')
			tmpTimeSignatures.push([parseInt(tmpTimeSig[0])/parseInt(tmpTimeSig[1]), gsMusicObject.timeSignatures[i]])
		}
		// console.log(tmpTimeSignatures);
		tmpTimeSignatures = tmpTimeSignatures.filter(([dec, timeS])=> { // what are we filtering here?
			// timeS = timeS.split('/').map(a=>parseInt(a)%2)
			timeS = timeS.split('/').map(a=>parseInt(a))
			// console.log(timeS)
			return timeS[0] <= timeS[1]
			// console.log(timeS)
			// console.log( timeS.reduce((a, b) => a + b, 0))
			// return timeS.reduce((a, b) => a + b, 0) > 0 
			// item = item.split("/").map(fractions => parseFloat(fractions) ); console.log(item)
		})
		// process.exit();
		tmpTimeSignatures = tmpTimeSignatures.sort((a,b)=>b[0] - a[0]) // sort descending
		// console.log(tmpTimeSignatures);
		// process.exit();
		// tmpTimeSignatures = tmpTimeSignatures.sort((a,b)=>parseInt(a.split("/")[0])/parseInt(a.split("/")[1]) - parseInt(b.split("/")[0])/parseInt(b.split("/")[1]))
		// console.log(tmpTimeSignatures);process.exit();
		console.log(this.tmpTimeSignatures)
		this.header.timeSignature = tmpTimeSignatures[0][1]
		// process.exit();
